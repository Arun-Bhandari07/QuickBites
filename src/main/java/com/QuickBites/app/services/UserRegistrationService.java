...
public class UserRegistrationService {
	...
	public UserRegistrationService(PendingUserRepository pendingUserRepo, UserRepository userRepo,
			UserRoleRepository userRoleRepo, FileService fileService, DeliveryAgentRepository agentRepo,
			AdminNotificationService adminNotificationService, MailService mailService) {
	...
	public void registerCustomer(String email) {
		...
		try {
			...
			throw new ResourceNotFoundException(
					"System Configuration Error:The role ROLE_CUSTOMER is not defined in the database.");
	...
	public void registerAgent(String email) {
		...
		if (!pendingUser.isAdminApproved() || !pendingUser.isOTPVerified()) {
			throw new RegistrationException(
					"Cannot register agent: The user has not been approved by an admin or has not completed OTP verification.");
		}
		...
		try {
			...
			throw new ResourceNotFoundException(
					"System Configuration Error: A required user role ('ROLE_DELIVERYAGENT' or 'ROLE_CUSTOMER') is not defined in the database");
		}

		String newCitizenshipFront, newCitizenshipBack, newLicense;
		try {
			newCitizenshipFront =fileService.moveFile(pendingUser.getCitizenshipPhotoFront());
			newCitizenshipBack =  fileService.moveFile(pendingUser.getCitizenshipPhotoBack());
			newLicense    =    fileService.moveFile(pendingUser.getDriverLicense());
		} catch (IOException ex) {
			throw new FileStorageException("Failed to move required documents during agent registration", ex);
		}

		DeliveryAgent agent = new DeliveryAgent();
		agent.setCitizenshipPhotoFront(newCitizenshipFront);
		agent.setCitizenshipPhotoBack(newCitizenshipFront);
		agent.setDrivingLicense(newLicense);
